...
static void a1000_patch_post(struct framebuffer_device_t *dev)
{
    ...
    a1000_fbopen();
    if (!dev || dev->post == a1000_overlay_post)
        return;
    /* Осечки тут были молчаливыми: в логе оставалось только "фабрика gralloc
     * перехвачена", и почему кадр не поехал -- не видно. Говорим вслух. */
    if (g_fbfd < 0) {
        ALOGE("post НЕ перехвачен: /dev/graphics/fb0 не открылся (%s)",
              strerror(errno));
        return;
    }
    if (ioctl(g_fbfd, FBIOGET_VSCREENINFO, &vi) != 0) {
        ALOGE("post НЕ перехвачен: FBIOGET_VSCREENINFO -> %s", strerror(errno));
        return;
    }
                              ...
                              struct hw_device_t **device)
{
    ...
    /* Открытий у gralloc за процесс единицы, поэтому печатаем каждое: без этого
     * не отличить "fb0 не открывали вовсе" от "открыли, но хук не встал". */
    ALOGE("gralloc open(\"%s\") -> %d", id ? id : "(null)", rc);
    if (rc == 0 && device && *device && id && strcmp(id, GRALLOC_HARDWARE_FB0) == 0)
        a1000_patch_post((struct framebuffer_device_t *)*device);
    return rc;
}

/* Подменить methods->open у УЖЕ полученного модуля gralloc. */
static void a1000_patch_factory(const struct hw_module_t *m)
{
    hw_open_t *slot;
    long page = sysconf(_SC_PAGESIZE);
    void *pg;

    if (g_real_hw_open || !m || !m->methods)
        return;
...
/* ПОЧЕМУ ЭТОГО МАЛО. Ленивый вход ниже цепляет фабрику на первом ioctl процесса,
 * а первый ioctl случается уже ВНУТРИ framebuffer_open (там читается
 * FBIOGET_VSCREENINFO). То есть к единственному open("fb0") за жизнь процесса мы
 * опаздываем ровно на этот вызов: в логе оставалось "фабрика перехвачена" и
 * ни одного "gralloc open(...)", а post так и не подменялся.
 *
 * Поэтому перехватываем сам hw_get_module. libui_shim.so стоит ПЕРВЫМ в
 * DT_NEEDED и у gralloc.sc8830.so, и у hwcomposer.sc8830.so, поэтому их вызов
 * попадает сюда, а не в libhardware, и фабрика оказывается подменена ДО того,
 * как блоб успеет открыть fb-устройство. */
/* Запасной, ленивый путь: если блоб получил модуль в обход нашего
 * hw_get_module (например, уже держал его), цепляемся на первом ioctl. */
static void a1000_hook_gralloc_factory(void)
{
    ...
    if (!get_mod || get_mod(GRALLOC_HARDWARE_MODULE_ID, &m) != 0)
        return;
    a1000_patch_factory(m);
}

extern "C" int hw_get_module(const char *id, const struct hw_module_t **module)
{
    typedef int (*hw_get_module_t)(const char *, const struct hw_module_t **);
    static hw_get_module_t real = NULL;
    int rc;

    if (!real)
        real = (hw_get_module_t)dlsym(RTLD_NEXT, "hw_get_module");
    if (!real)
        return -1;
    rc = real(id, module);
    if (rc == 0 && id && module && *module &&
        strcmp(id, GRALLOC_HARDWARE_MODULE_ID) == 0) {
        g_gralloc = (const gralloc_module_t *)*module;
        a1000_patch_factory(*module);
    }
    return rc;
}
