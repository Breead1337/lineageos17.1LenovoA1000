...
/* ===== GraphicBuffer::lock(uint32 usage, void** vaddr) =====
 * legacy: _ZN7android13GraphicBuffer4lockEjPPv
 * В десятке к lock добавили выходные bytesPerPixel/bytesPerStride, из-за
 * чего манглинг стал ...lockEjPPvPiS3_ и старое имя пропало. Блобу
 * hwcomposer.sc8830.so нужно именно старое — без него dlopen блоба падает
 * и композитор откатывается на framebuffer HAL. */
extern "C" status_t _sprdshim_gb_lock(GraphicBuffer* self,
        uint32_t usage, void** vaddr)
    asm("_ZN7android13GraphicBuffer4lockEjPPv");
extern "C" status_t _sprdshim_gb_lock(GraphicBuffer* self,
        uint32_t usage, void** vaddr) {
    return self->lock(usage, vaddr);
}
