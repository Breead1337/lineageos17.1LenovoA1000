# A1000: цели этого файла — сборочные проверки, на устройство не едут.
# POLICYVERS у нас понижен до 28 ради ядра 3.10, но предсобранные CIL
# старых версий содержат xperm-правила, невыразимые в 28. Собираем
# проверки штатной версией.
A1000_TEST_POLICYVERS := 30
...
$(built_$(version)_plat_sepolicy): $($(version)_plat_policy.conf) $(HOST_OUT_EXECUTABLES)/checkpolicy \
  ...
  $(built_sepolicy_neverallows)
	...
	$(hide) $(CHECKPOLICY_ASAN_OPTIONS) $(HOST_OUT_EXECUTABLES)/checkpolicy -M -C -c \
		$(A1000_TEST_POLICYVERS) -o $@ $<
	$(hide) cat $(PRIVATE_ADDITIONAL_CIL_FILES) >> $@
	$(hide) $(HOST_OUT_EXECUTABLES)/secilc -m -M true -G -c $(A1000_TEST_POLICYVERS) $(PRIVATE_NEVERALLOW_ARG) $@ -o $@ -f /dev/null
...
$(built_plat_cil) $($(version)_mapping.cil) $($(version)_nonplat)
	$(hide) $(HOST_OUT_EXECUTABLES)/secilc -m -M true -G -N -c $(A1000_TEST_POLICYVERS) \
