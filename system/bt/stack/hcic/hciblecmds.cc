...
/* A1000: LE Rand с контроллера, который LE не умеет.
 * Команда HCI LE Rand делает ровно одно — отдаёт 8 случайных байт. Наш чип
 * (2.1+EDR) её не знает и отвечает ненулевым статусом, а исходный CHECK на
 * это падал, унося с собой весь стек ещё на инициализации (BTM_SecRegister ->
 * btm_ble_reset_id генерит Identity Root безусловно). Берём байты у ядра:
 * энтропия та же, а LE-крипты на этом устройстве всё равно нет. */
static void btsnd_hcic_ble_rand_from_host(base::Callback<void(BT_OCTET8)> cb) {
  BT_OCTET8 rand = {0};
  int fd = open("/dev/urandom", O_RDONLY);
  bool ok = false;
  if (fd >= 0) {
    ssize_t got = TEMP_FAILURE_RETRY(read(fd, rand, BT_OCTET8_LEN));
    ok = (got == (ssize_t)BT_OCTET8_LEN);
    close(fd);
  }
  if (!ok) {
    /* Молча отдать нули нельзя: это предсказуемый "случайный" ключ. */
    LOG(ERROR) << __func__ << ": /dev/urandom unavailable, LE Rand failed";
    return;
  }
  cb.Run(rand);
}
...
void btsnd_hcic_ble_rand(base::Callback<void(BT_OCTET8)> cb) {
  if (!controller_get_interface()->supports_ble()) {
    /* A1000: спрашивать нечего — LE-части в контроллере нет. */
    btsnd_hcic_ble_rand_from_host(std::move(cb));
    return;
  }
  ...
  btu_hcif_send_cmd_with_cb(
      ...
      base::Bind(
          [](base::Callback<void(BT_OCTET8)> cb, uint8_t* param,
             uint16_t param_len) {
            if (param_len < 1 + BT_OCTET8_LEN || param[0] != 0) {
              /* A1000: апстрим тут делал CHECK и ронял процесс. Контроллер,
               * который заявил LE, но LE Rand не выполнил — повод отработать
               * на своей энтропии, а не убить Bluetooth целиком. */
              LOG(ERROR) << "LE Rand failed (status "
                         << (param_len ? (int)param[0] : -1) << ", len "
                         << param_len << "), using host RNG";
              btsnd_hcic_ble_rand_from_host(std::move(cb));
              return;
            }
