...
static void btu_hcif_encryption_change_evt(uint8_t* p) {
  ...
  } else if (!controller_get_interface()->supports_read_encryption_key_size()) {
    /* A1000: контроллер не умеет Read Encryption Key Size (команда из BT 3.0,
     * чип 2.1+EDR). Отправить её нельзя: на Command Status "Unknown HCI
     * Command" колбэк не позовётся вообще, и стек навсегда останется без
     * btm_sec_encrypt_change, то есть подключение подвиснет. Ведём себя как
     * до-2019 стек: считаем шифрование включённым без проверки длины ключа. */
    HCI_TRACE_WARNING(
        "%s: encryption key size check skipped, controller does not support "
        "Read Encryption Key Size (handle: 0x%02x)",
        __func__, handle);
    btm_acl_encrypt_change(handle, status, encr_enable);
    btm_sec_encrypt_change(handle, status, encr_enable);
  } else {
...
static void btu_hcif_encryption_key_refresh_cmpl_evt(uint8_t* p) {
  ...
  if (status != HCI_SUCCESS || BTM_IsBleConnection(handle)) {
    ...
    /* A1000: см. комментарий в btu_hcif_encryption_change_evt. */
    HCI_TRACE_WARNING(
        "%s: encryption key size check skipped, controller does not support "
        "Read Encryption Key Size (handle: 0x%02x)",
        __func__, handle);
