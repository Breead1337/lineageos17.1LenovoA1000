...
static bool filter_incoming_event(BT_HDR* packet) {
  ...
  // A1000: событие Command Status отдано в future и теперь принадлежит ему,
  // освобождать его здесь нельзя. Объявлено до всех goto intercepted.
  bool status_given_to_future = false;
  ...
  if (event_code == HCI_COMMAND_COMPLETE_EVT) {
    ...
    if (!wait_entry) {
      ...
      if (wait_entry->status_callback) {
        ...
        // A1000: команда ждала Command Complete через future (AWAIT_COMMAND в
        // controller.cc), а контроллер ответил Command Status — так наш
        // WCN-чип отвечает на VSC 0xFD53, которого не знает. У futured-команды
        // status_callback всегда NULL, и апстрим тут просто выбрасывал
        // событие: future никто не закрывал, future_await висел вечно и
        // инициализация BT замирала насмерть. Отдаём событие в future —
        // read_command_complete_header разберётся и вернёт NULL.
        LOG_WARN(LOG_TAG,
                 "%s command status (0x%02x) for futured command opcode 0x%04x",
                 __func__, status, opcode);
        future_ready(wait_entry->complete_future, packet);
        status_given_to_future = true;
      }
    }
...
intercepted:
  if (wait_entry) {
    ...
    if ((event_code == HCI_COMMAND_STATUS_EVT && !status_given_to_future) ||
