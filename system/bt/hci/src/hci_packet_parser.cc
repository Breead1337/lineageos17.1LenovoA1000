...
static uint8_t* read_command_complete_header(BT_HDR* response,
  ...
  // A1000: контроллер может ответить Command Status вместо Command Complete —
  // так наш WCN-чип отвечает на неизвестную ему VSC 0xFD53. Ронять процесс
  // из-за этого нельзя: возвращаем NULL и пусть вызывающий решает сам.
  // parse_ble_read_offload_features_response уже умеет NULL; те, кому ответ
  // обязателен, упрутся в свой собственный CHECK(stream != NULL).
  if (event_code != HCI_COMMAND_COMPLETE_EVT) {
    LOG_ERROR(LOG_TAG, "%s: expected command complete, got event 0x%02x",
              __func__, event_code);
    return NULL;
  }
