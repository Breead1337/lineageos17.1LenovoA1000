...
// A1000: умеет ли контроллер Read Encryption Key Size. Команда из BT 3.0,
// наш 2.1-чип её не знает. Значение до start_up — false, то есть "не умеет":
// так безопаснее, чем случайно отправить команду до инициализации.
static bool read_encryption_key_size_supported;
...
static future_t* start_up(void) {
  ...
  // A1000: апстрим тут делает LOG(FATAL) — это обязательная с 2019 года
  // защита от KNOB (CVE-2019-9506). Наш WCN-чип 2.1+EDR команды Read
  // Encryption Key Size не знает (supported_commands[20] = 0x0c), и с FATAL
  // Bluetooth на этом устройстве не включится в принципе. Запоминаем флаг и
  // едем дальше: btu_hcif пропустит сам запрос длины ключа, а вместе с ним и
  // проверку. Плата — BR/EDR снова уязвим к KNOB.
  read_encryption_key_size_supported =
      HCI_READ_ENCR_KEY_SIZE_SUPPORTED(supported_commands);
  if (!read_encryption_key_size_supported) {
    LOG(WARNING) << __func__
                 << ": controller does not support Read Encryption Key Size;"
                 << " BR/EDR encryption key length will NOT be verified"
                 << " (KNOB CVE-2019-9506 mitigation disabled)";
  }
...
static const bt_device_features_t* get_features_ble(void) {
  CHECK(readable);
  // A1000: LE в контроллере нет — отвечаем "ничего не поддерживается",
  // а не падаем.
  if (!ble_supported) return &features_ble;
  return &features_ble;
}
...
static const uint8_t* get_ble_supported_states(void) {
  ...
  // A1000: LE в контроллере нет — отвечаем "ничего не поддерживается",
  // а не падаем.
  if (!ble_supported) return ble_supported_states;
...
static bool supports_read_encryption_key_size(void) {
  // Намеренно без CHECK(readable): этот геттер зовут из обработчиков HCI-событий,
  // в том числе когда стек уже гасится. До start_up вернётся false — а false
  // означает "не спрашивать контроллер", что безопасно в любой момент.
  return read_encryption_key_size_supported;
}
