...
Result<Success> SetMmapRndBitsAction(const BuiltinArguments&) {
    /* A1000: ядро 3.10 не знает про mmap_rnd_bits: ручка /proc/sys/vm/mmap_rnd_bits
     * появилась с ARCH_MMAP_RND_BITS в 4.x (см. список коммитов ниже).
     * Раз её нет вовсе — это старое ядро, а не сбой: предупреждаем и
     * продолжаем. Если ручка есть, но записать не вышло — прежний FATAL. */
    if (access(MMAP_RND_PATH, F_OK) != 0 && access(MMAP_RND_COMPAT_PATH, F_OK) != 0) {
        LOG(WARNING) << "mmap_rnd_bits: ядро не поддерживает, пропускаем";
        return Success();
    }
