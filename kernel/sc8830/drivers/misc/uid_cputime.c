 ...
 *
 * A1000: перенос в ядро 3.10. Отличие от оригинала одно:
 * for_each_process_thread() появился в 3.16, здесь его заменяет пара
 * do_each_thread()/while_each_thread() под rcu_read_lock().
 */
...
static int uid_stat_show(struct seq_file *m, void *v)
{
	...
	do_each_thread(task, t) {
		...
		uid_entry->active_utime += t->utime;
		uid_entry->active_stime += t->stime;
