...
void sprdchg_init(struct sprd_battery_platform_data *pdata)
{
	...
	/*wdm---*/
	printk("%s;adc_cal=%d;%d;%d;%d;%d;\n", __func__,adc_cal.p0_vol ,adc_cal.p0_adc,
                adc_cal.p1_vol,adc_cal.p1_adc,adc_cal.cal_type);
	/*---*/
...
int sprdchg_read_temp(void)
{
	...
	if (pbat_data->temp_support) {
		...
		/*wdm---*/
                //adc_most = 2183;
                adc_most = 3600;
                if(val ==0) //no pullup 10k
                {
                        printk(KERN_ERR "sprdchg: sprdchg_read_temp no pullup 10k: 21 degree %d;\n", val);
                        return 210;
                }
                //if(val > (adc_most +120)) //have pullup ,but no ntc  ,1.8v adc is 2397(real) 2351-2155(test)
                //{
                //      printk(KERN_ERR "sprdchg: sprdchg_read_temp have pullup , but no ntc: 22 degree %d;\n", val);
                //      return 220;
                //}
                if(val > adc_most ) //have pullup and pulldown ,but no ntc 
                {
                        printk(KERN_ERR "sprdchg: sprdchg_read_temp have pullup and pulldown, but no ntc: 22 degree %d;\n", val);
                        return -200;
                }
		/*---*/
                //voltage mode
		...
		/*wdm---*/
		store_value = sprdfgu_read_batcurrent();
                printk("in charging,temp pin adjusted before val is %d,after val is %d, now current is %d!\n",val, val-store_value*20/1000,store_value);
                val -= store_value*20/1000;
                store_value = sprdchg_search_temp_tab(val);
                printk("The temp is %d!\n", store_value);
                return store_value;
		/*---*/
                //return sprdchg_search_temp_tab(val);
	} else {
		printk(KERN_ERR "sprdchg: sprdchg_read_temp temp_support==0;\n");
		return 200;
	}
}
...
{
	if((sprd_ext_ic_op != NULL) && (sprd_ext_ic_op->ext_set_chg_cur != NULL))
	{
		sprd_ext_ic_op->ext_set_chg_cur(chg_current);
	}
	else {
	uint32_t temp;
...
void sprdchg_set_cccvpoint(unsigned int cvpoint,unsigned int cccv)
{
	if((sprd_ext_ic_op != NULL) && (sprd_ext_ic_op->ext_set_cccvpoint != NULL))
        {
                sprd_ext_ic_op->ext_set_cccvpoint(cccv);
        }
        else {
...
{
	if((sprd_ext_ic_op != NULL) && (sprd_ext_ic_op->ext_stop_charge != NULL))
        {
                sprd_ext_ic_op->ext_stop_charge();
        }
        else {
...
{
	if((sprd_ext_ic_op != NULL) && (sprd_ext_ic_op->ext_start_charge != NULL))
        {
                sprd_ext_ic_op->ext_start_charge();
        }
        else {
#if defined(CONFIG_ARCH_SCX15) ||defined(CONFIG_ADIE_SC2723S) ||defined(CONFIG_ADIE_SC2723)
...
static uint32_t _sprdchg_read_chg_current(void)
{
	...
	if (isense > vbat) {
		...
		/* A1000: не ошибка, а «тока нет» — сыпалось в лог постоянно.
		 * Включить при разборе зарядки: dynamic_debug. */
		pr_debug("chg_current: isense:%d vbat:%d\n", isense, vbat);
		return 0;
	}
}
