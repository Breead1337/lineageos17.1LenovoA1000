...
#define VOREG4P32 41  // refer to table 3
#define VOREG4P34 42  // refer to table 3
#define VOREG4P36 43  // refer to table 3
#define VOREG4P38 44  // refer to table 3
...
{
	pr_debug("fan5405: reset 32s timer\n");
	pr_debug("fan5405: i2c errors read=%d write=%d\n", read_err_count, write_error_count);
...
{
	pr_debug("fan5405: reset 32s timer (ap)\n");
	pr_debug("fan5405: i2c errors read=%d write=%d\n", read_err_count, write_error_count);
    fan5405_SetValue(FAN5405_REG_CONTROL0, FAN5405_TMR_RST_OTG,FAN5405_TMR_RST_OTG_SHIFT, RESET32S);
    mod_timer(&t32s_timer, jiffies + (T32S_RESET_INTERVAL * HZ));
}
...
//reg 6
	fan5405_SetValue(FAN5405_REG_SAFETY, FAN5405_VSAFE,FAN5405_VSAFE_SHIFT, VSAFE4P36);  // VSAFE = 4.36V
	fan5405_SetValue(FAN5405_REG_SAFETY, FAN5405_ISAFE, FAN5405_ISAFE_SHIFT, ISAFE850);  // ISAFE = 850mA (68mOhm)
//reg 1
...
//reg 2	
	fan5405_SetValue(FAN5405_REG_OREG, FAN5405_OREG,FAN5405_OREG_SHIFT, VOREG4P34);  //OREG = 4.34V
//reg 5
...
//reg 5
	fan5405_SetValue(FAN5405_REG_SP_CHARGER, FAN5405_VSP,FAN5405_VSP_SHIFT, VSP4P293);
...
void fan5405_TA_StartCharging(void)
{
    ...
    fan5405_SetValue(FAN5405_REG_IBAT, FAN5405_IOCHARGE,FAN5405_IOCHARGE_SHIFT, IOCHARGE850);  //850mA
    fan5405_SetValue(FAN5405_REG_IBAT, FAN5405_ITERM,FAN5405_ITERM_SHIFT, ITERM97);  //97ma
    fan5405_SetValue(FAN5405_REG_CONTROL1, FAN5405_IINLIM, FAN5405_IINLIM_SHIFT,NOLIMIT);  // no limit
    fan5405_SetValue(FAN5405_REG_CONTROL1, FAN5405_TE,FAN5405_TE_SHIFT, ENTE);
...
/*wdm*/
void fan5405_set_chg_cur(uint32_t chg_current)
{
	int current_value = 0;
	if(chg_current >= 1450)
		current_value = IOCHARGE1450;
	else if(chg_current >= 1350)
		current_value = IOCHARGE1350;
	else if(chg_current >= 1150)
                current_value = IOCHARGE1150;
	else if(chg_current >= 1050)
                current_value = IOCHARGE1050;
	else if(chg_current >= 850)
                current_value = IOCHARGE850;
	else if(chg_current >= 750)
                current_value = IOCHARGE750;
	else if(chg_current >= 650)
                current_value = IOCHARGE650;
	else 
                current_value = IOCHARGE550;
	fan5405_SetValue(FAN5405_REG_IBAT, FAN5405_IOCHARGE,FAN5405_IOCHARGE_SHIFT, current_value);  //850mA
}

void fan5405_set_cccvpoint(unsigned int cvpoint)
{
	unsigned int cv = 0;
	
	if(cvpoint >= 4380)
		cv = VOREG4P38;
	else if(cvpoint >= 4360)
		cv = VOREG4P36;
	else if(cvpoint >= 4340)
                cv = VOREG4P34;
	else 
                cv = VOREG4P32;

	fan5405_SetValue(FAN5405_REG_OREG, FAN5405_OREG,FAN5405_OREG_SHIFT, cv);  //OREG = 4.34V
}

uint32_t fan5405_get_cccvpoint(void)
{
	return 0;
}

void fan5405_start_charge(void)
{
    	fan5405_SetValue(FAN5405_REG_CONTROL1, FAN5405_CE_N,FAN5405_CE_N_SHIFT, ENCHARGER);
}

void fan5405_stop_charge(void)
{
	fan5405_SetValue(FAN5405_REG_CONTROL1, FAN5405_CE_N,FAN5405_CE_N_SHIFT, DISCHARGER);
}
/*wdm+*/
...
/*wdm*/
	.ext_set_chg_cur = fan5405_set_chg_cur,
	.ext_set_cccvpoint = fan5405_set_cccvpoint,
	.ext_get_cccvpoint = fan5405_get_cccvpoint,
	.ext_start_charge = fan5405_start_charge,
	.ext_stop_charge = fan5405_stop_charge,
/*wdm+*/
};
/*wdm+*/
int fan5405_exist = 0;
