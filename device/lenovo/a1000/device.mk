...
# Установка приложений. AOSP по умолчанию ставит install=speed-profile, а
# профиля у только что поставленного приложения нет — dex2oat в этом случае компилирует ВСЁ.
# Замер на RuStore 86 МБ (одно только копирование apk — 16 с):
#     speed-profile  125 с  (dex2oat 103 с)
#     quicken        105 с  (dex2oat  81 с)
#     extract         89 с  (dex2oat  58 с)
# extract только распаковывает dex, без проверки и компиляции: проверка
# уезжает в рантайм, а нормальную компиляцию делает pm.dexopt.bg-dexopt
# (speed-profile), когда телефон стоит на зарядке без дела. Форсировать вручную:
#     cmd package bg-dexopt-job
PRODUCT_PROPERTY_OVERRIDES += pm.dexopt.install=extract
...
PRODUCT_COPY_FILES += \
    ...
    $(LOCAL_PATH)/rootdir/etc/fstab.sc8830:ramdisk/fstab.sc8830 \
    $(LOCAL_PATH)/rootdir/etc/fstab.sc8830:ramdisk/fstab.unknown \
...
PRODUCT_PROPERTY_OVERRIDES += \
    ...
    debug.sf.a1000_pan=0 \
    debug.sf.a1000_zerocopy=1 \
...
PRODUCT_COPY_FILES += \
    ...
    $(LOCAL_PATH)/rootdir/etc/a1000_earlylog.rc:system/etc/init/a1000_earlylog.rc \
...
# Картинки ставит модуль из vendor/lineage/charger (без него анимации нет).
