...
#endif
    // A1000_IMEI: вендорная libreference-ril_sp.so не реализует запрос 98
...
int radio::getIccCardStatusResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        /* A1000: индекс вне диапазона — это НЕ повод выбросить весь ответ.
...
int radio::getSignalStrengthResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        populateResponseInfo(responseInfo, serial, responseType, e);
        SignalStrength signalStrength = {};
        /* A1000: короткие структуры v5/v6 тоже годятся, см.
...
// A1000: вендорный блоб пользуется устаревшими состояниями радио.
...
int radio::currentSignalStrengthInd(int slotId,
                                    ...
                                    void *response, size_t responseLen) {
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
        /* A1000: короткие структуры v5/v6 тоже годятся. */
        if (response == NULL || responseLen < sizeof(RIL_SignalStrength_v5)) {
...
void radio::registerService(RIL_RadioFunctions *callbacks, CommandInfo *commands) {
    ...
    /* A1000: шесть потоков вместо одного. AT+SFUN=4 у этого модема отвечает
